...
def rows_from_batch(batch: pa.RecordBatch) -> list[dict[str, object]]:
    data = batch.to_pydict()
    return [
        {"video_path": video_path, "chunk": chunk, "embedding": embedding}
        for video_path, chunk, embedding in zip(data["video_path"], data["chunk"], data["embedding"], strict=True)
    ]


def print_report(dropped: list[str], inserted: int, row_count: int) -> None:
...
def main() -> None:
    ...
    print_report(dropped, inserted, row_count)
